...
async function main() {
  const ap = await bcrypt.hash('beeyond2024', 10)
  const mp = await bcrypt.hash('merchant2024', 10)
  await prisma.user.upsert({ where: { username: 'admin' }, update: {}, create: { username: 'admin', password: ap, name: 'Administrator', role: 'admin' } })
