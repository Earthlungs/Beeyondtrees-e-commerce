  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 360px), 1fr))', gap: '20px' }}>
        ...
        {/* Livestock quick access */}
        {isAdmin && (
          <Card style={{ borderColor: '#A89F91' }}>
            <CardHeader style={{ paddingBottom: '8px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <CardTitle style={{ fontSize: '16px', color: "var(--admin-text)", display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <PawPrint size={18} /> Livestock
                </CardTitle>
                <button onClick={() => setLivestockExpanded((v) => !v)} style={{ fontSize: '13px', color: '#6B7D5C', background: 'none', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '4px', padding: 0 }}>
                  Manage {livestockExpanded ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
                </button>
              </div>
            </CardHeader>
            <CardContent>
              {livestockExpanded ? (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  {[
                    { label: 'Livestock', href: '/admin/livestock', icon: PawPrint },
                    { label: 'Housing', href: '/admin/livestock/housing', icon: Warehouse },
                    { label: 'Feed', href: '/admin/livestock/feed', icon: Wheat },
                    { label: 'Reports', href: '/admin/livestock/reports', icon: BarChart3 },
                  ].map((item) => (
                    <Link key={item.href} href={item.href} style={{ textDecoration: 'none' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '10px', padding: '10px', backgroundColor: 'var(--admin-bg)', borderRadius: '8px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                <p style={{ color: "var(--admin-muted)", textAlign: 'center', padding: '20px', fontSize: '14px' }}>Housing, feed &amp; animal/herd production</p>
              )}
            </CardContent>
          </Card>
        )}
