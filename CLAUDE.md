...
**Agro Forestry** (`/admin/agro-forestry`, `src/app/api/agro-forestry/*`) is a read-only mirror of the EarthLungs reforestation Supabase project — the farmer register plus beehive/seedling handovers. Its six Prisma models (`Farmer`, `Beehive`, `Seedlings`, `ItemDisbursement`, `DisbursedBeehive`, `DisbursedSeedling`) deliberately break this schema's cuid/PascalCase conventions: they use serial `Int` ids and `@map`/`@@map` to the *source* table and column names, so rows import by their original id with the foreign keys between them intact. Populate it with `npm run import:agroforestry` (needs `SOURCE_SUPABASE_URL` + a `SOURCE_SUPABASE_KEY` with read access; `--dry-run` to count first). `Beehive.baitingReportId` is a soft reference — the source `BaitingReport` subsystem is not mirrored, so no FK, same convention as `Batch.lpoId`. Note the source is Prisma-managed too, and its `public` schema has had the Supabase role grants stripped (`42501` for *both* `anon` and `service_role`), so the import cannot run until `GRANT USAGE ON SCHEMA public TO service_role` is restored over there.
